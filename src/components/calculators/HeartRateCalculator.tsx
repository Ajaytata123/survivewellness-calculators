...
const HeartRateCalculator: React.FC<HeartRateCalcProps> = ({ unitSystem }) => {
  ...
  const downloadResults = () => {
    ...
    downloadResultsAsCSV(results, "Heart-Rate-Zones");
  ...
  const shareLink = () => {
    ...
    const link = createShareableLink("heartrate", params);
      ...
      {maxHr && hrZones && (
        <div className="bg-gray-50 p-4 rounded-md">
          <div className="text-center mb-4">
            ...
            <p className="text-3xl font-bold my-2">Max HR: {maxHr} bpm</p>
            {userName && <p className="text-sm">Results for: {userName}</p>}
          </div>
